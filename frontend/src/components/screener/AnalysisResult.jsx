...
const impactColors = {
  High:   { bg: 'rgba(251,113,133,0.1)',  border: 'rgba(251,113,133,0.3)',  text: '#FB7185' },
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-1">
        ...
        <span className="text-xs font-mono font-semibold text-indigo-400">{value}%</span>
      </div>
...
const PLAN_PHASES = [
  { key: 'this_week', label: 'This Week',  color: '#FB7185', icon: '⚡' },
  { key: 'day_30',   label: '30 Days',    color: '#FBBF24', icon: '📅' },
  { key: 'day_60',   label: '60 Days',    color: '#818CF8', icon: '🚀' },
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="card p-4 flex items-center gap-3"
        ...
        <p className="font-body text-slate-300 text-sm">{verdict}</p>
      </div>
      ...
      <div className="grid md:grid-cols-[auto_1fr] gap-5">
        ...
        <Section title="Score Breakdown">
          <div className="flex flex-col gap-3">
            <ScoreBar label="Skills Match"           value={score_breakdown?.skills_match ?? 0} />
            <ScoreBar label="Experience Relevance"   value={score_breakdown?.experience_relevance ?? 0} />
            <ScoreBar label="Education Fit"          value={score_breakdown?.education_fit ?? 0} />
            <ScoreBar label="Keyword Density"        value={score_breakdown?.keyword_density ?? 0} />
          </div>
        </Section>
      </div>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        <Section title={`✅ Matched Skills (${(matched_skills || []).length})`}>
          <div className="flex flex-wrap gap-2">
            {(matched_skills || []).map((s, i) => (
              <span key={i} className="tag-matched stagger-item">{s}</span>
            ))}
        ...
        <Section title={`❌ Missing Skills (${(missing_skills || []).length})`}>
          <div className="flex flex-col gap-2">
            {(missing_skills || []).map((s, i) => (
              <div key={i} className="stagger-item flex items-start gap-2 p-2 rounded-xl"
                style={{ background: 'rgba(251,113,133,0.04)', border: '1px solid rgba(251,113,133,0.08)' }}>
                <span className="tag-missing mt-0.5">{s.skill}</span>
                <div className="flex-1">
                  <ImpactBadge level={s.importance} />
                  <p className="text-xs text-slate-500 mt-1 font-body">{s.why_needed}</p>
                </div>
              </div>
            ))}
          </div>
      ...
      <div className="grid md:grid-cols-2 gap-5">
        <Section title="💪 Your Strengths">
          <ul className="flex flex-col gap-2">
            {(strengths || []).map((s, i) => (
              <li key={i} className="stagger-item flex items-start gap-2 text-sm font-body text-slate-300">
        ...
        <Section title="🔧 Improvements to Make">
          <div className="flex flex-col gap-3">
            {(improvements || []).map((imp, i) => (
              <div key={i} className="stagger-item p-3 rounded-xl"
                style={{ background: 'rgba(10,15,36,0.6)', border: '1px solid rgba(99,102,241,0.08)' }}>
                <div className="flex items-center gap-2 mb-1">
                  <span className="font-display font-semibold text-slate-200 text-sm">{imp.area}</span>
                  <ImpactBadge level={imp.impact} />
                </div>
                <p className="text-xs text-slate-500 font-body">{imp.suggestion}</p>
              </div>
            ))}
          </div>
        </Section>
              ...
              return (
                <div key={key} className="rounded-xl overflow-hidden"
                  ...
                  <ul className="p-3 flex flex-col gap-2">
                    {items.map((item, j) => (
                      <li key={j} className="flex items-start gap-2 text-xs font-body text-slate-400">
      ...
      {(ats_tips || []).length > 0 && (
        <Section title="🤖 ATS Optimisation Tips">
          <div className="grid sm:grid-cols-2 gap-2">
            {ats_tips.map((tip, i) => (
              <div key={i} className="stagger-item flex items-start gap-3 p-3 rounded-xl"
                ...
                <p className="text-sm font-body text-slate-400">{tip}</p>
              </div>
            ))}
          </div>
        </Section>
